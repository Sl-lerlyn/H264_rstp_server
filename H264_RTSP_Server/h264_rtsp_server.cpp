                        ...
                        struct RtpPacket* rtpPacket, uint8_t* frame, uint32_t frameSize)
{
    ...
    if (frameSize <= RTP_MAX_PKT_SIZE){ // nalu长度小与最大包场， 单一nalu单元模式
    ...
    while (true){
        ...
        if (!strcmp(method, "PLAY")){
            ...
            while (true){
                ...
                if (startCode3(frame))  startCode = 3;
                else  startCode = 4;
...
int main(int argc, char* argv[])
{
    ...
    while (true){
        ...
        doClient(clientSockfd, (const char*)clientIp, clientPort, serverRtpSockfd, serverRtcpSockfd);
    }
